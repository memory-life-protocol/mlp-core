...
async function buildAdapters(): Promise<{
  ...
  if (IS_PRODUCTION) {
    ...
    const { AnthropicEmbeddingAdapter } = await import(
      './adapters/anthropic-embedder.js'
    )
    const { AnthropicExtractionAdapter } = await import(
      './adapters/anthropic-extractor.js'
    )
